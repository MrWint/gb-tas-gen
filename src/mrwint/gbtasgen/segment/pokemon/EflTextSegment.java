...
//      EflUtil.runInputPastAddress(skipMove, curGb.pokemon.printLetterDelayDoneAddress);
		  if (hasMoreText(textSpeed)) {
		    EflUtil.runToNextInputFrame(skipMove); // this is guaranteed to be for the next letter delay through hasMoreText()
...
//    System.out.println("hasMoreText steps: "+curGb.currentStepCount);
    ...
    while (true) {
      int add = EflUtil.runToAddressOrNextInputFrame(skipMove, inPrintLetterDelay ? curGb.pokemon.printLetterDelayDoneAddress : curGb.pokemon.printLetterDelayJoypadAddress);
      if (curGb.currentStepCount - initialSteps > textSpeed) {
...
//	}
	...
	private void skipToStart(StateBuffer startBuffer, State s) {
	  curGb.restore(s);
	  EflUtil.runToFirstInputFrameAfterAddress(skipMove, curGb.pokemon.printLetterDelayJoypadAddress); // PrintLetterDelay -> GetJoypadState call
