...
public class TextSegment extends Segment {
	...
	private void progress(State s, StateBuffer nextBuffer, StateBuffer goalBuffer, boolean first) {
		...
		if(!textSpeedInit) {
			textSpeedInit = true;
			textSpeed = Gb.readMemory(RomInfo.pokemon.optionsAddress) & RomInfo.pokemon.optionsTextSpeedMask;
		}
		if (textSpeed == 0) {
			State os = s;
			while (goToNextInput()) {
		...
		int add = State.step(0,RomInfo.pokemon.printLetterDelayDoneAddress,RomInfo.pokemon.readJoypadAddress); // .done, input
		if(add == RomInfo.pokemon.readJoypadAddress) {
			inputBeforeEnd = true;
			add = State.step(0,RomInfo.pokemon.printLetterDelayDoneAddress);
		}
		if(add == 0 && !first)
		...
		if((!first || add != 0) && !goToNextInput()) { // that was last call
			if(!finishLastFrame || !inputBeforeEnd) {
				...
				goalBuffer.addState(State.createState(true)); // frame finished (no inputs)
				s.restore();
				add = State.step(skipMove, RomInfo.pokemon.printLetterDelayDelayFrameAddress, RomInfo.pokemon.printLetterDelayDoneAddress); // before/after delaying frame -> keypress actually recognized
				if(add != RomInfo.pokemon.printLetterDelayDelayFrameAddress)
		...
		if(add != 0)
			add = State.step(skipMove, RomInfo.pokemon.printLetterDelayDelayFrameAddress); // delaying frame -> keypress actually recognized
		if(add != 0) {
			State.step(skipMove); // finish frame
			nextBuffer.addState(State.createState(true));
		} else
	...
	private boolean goToNextInput() {
		int add = State.step(0, RomInfo.pokemon.printLetterDelayJoypadAddress); // PrintLetterDelay -> GetJoypadState call
		if(add == 0)
			return false;
		add = State.step(0, RomInfo.pokemon.readJoypadAddress); // actual input reading instruction
		if(add == 0)
			return false;
		return true;
	}
	...
	private void skipToStart(StateBuffer startBuffer, State s) {
		s.restore();
		Util.runToFrameBeforeAddress(0, 0, RomInfo.pokemon.printLetterDelayJoypadAddress); // PrintLetterDelay -> GetJoypadState call
