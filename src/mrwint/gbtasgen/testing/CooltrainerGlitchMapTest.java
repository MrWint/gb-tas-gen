    ...
    // select ROM to use
	  curGb = new Gameboy(new RedRomInfo(), 0, false);
		...
		for (int map = MIN_MAP; map <= MAX_MAP; map++) {
			...
			int mapHeader = mapBankOffset + Util.getRomWordLE(mapHeaderPointers + 2*map);
			...
			int tilesetAdd = tilesetBankOffset + Util.getRomWordLE(tilesetHeaders + tileset*12 + 1);
			int tilesetColl = Util.getRomWordLE(tilesetHeaders + tileset*12 + 5);
			...
			int blocks = Util.getRomWordLE(mapHeader+3);
			...
			add = mapBankOffset + Util.getRomWordLE(add);
			...
			for (int loc=MIN_LOC;loc<=MAX_LOC;loc++) {
				if (add / 0x4000 != mapHeader / 0x4000)
					break;
				int blockAdd = Util.getRomWordLE(add);
				oy = rom[add+2];
				ox = rom[add+3];
				add += 4;
				init(ox,oy, 1); // down
