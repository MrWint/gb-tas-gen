...
public class Gen1Map extends Map {
	...
	private boolean isPassableFromTo(int fromx,int fromy, int tox,int toy) {
		...
		int curTileSet = Gb.readMemory(RomInfo.rom.curTilesetAddress);
		for(int c = RomInfo.rom.tilePairCollisionsLandAddress; memory[c] != 0xFF; c+=3) {
...
//		System.out.println("Trainer header at "+Util.toHex(trainerHeaderPointers));
		...
		for(int blockX=0;blockX<mapWidth+6;blockX++) {
			for(int blockY=0;blockY<mapHeight+6;blockY++) {
				int block = memory[RomInfo.rom.curBlockDataAddress + blockY*(mapWidth+6) + blockX];
				for(int y=0;y<4;y++) {
					for(int x=0;x<4;x++) {
		...
		collisionAddress = Util.getMemoryLittleEndian(RomInfo.rom.collisionDataAddress);
		...
		textDataPointer = Util.getWordAt(curAdd) + (mapHeaderBank-1)*0x4000;
	...
	private static boolean isSpriteHidden(int spriteIndex) {
		...
		int curAdd = RomInfo.rom.missableObjectListAddress; // W_MISSABLEOBJECTLIST
		int bitIndex = -1;
		...
		curAdd = RomInfo.rom.missableObjectFlagsAddress + (bitIndex/8); // W_MISSABLEOBJECTFLAGS
